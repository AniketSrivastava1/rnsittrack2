...
# Fix the weird double-newspaces from the bad UTF converion
content = re.sub(r'\n\s*\n', '\n', orig)

# 1. CSS
m_css = re.search(r'\s*\.main-content\s*\{\{.*?(?=\s*\.view-header\s*\{\{)', content, re.DOTALL)
if not m_css: print('CSS block not found'); sys.exit(1)
...
\n"""
content = content[:m_css.start()] + css_payload + content[m_css.end():]

# 2. HTML
m_html = re.search(r'\s*<div\s+class="main-content"\s*>\s*<div\s+id="content-view"', content)
if not m_html: print('HTML block not found'); sys.exit(1)
...
html_payload = """
    <div class="main-content">
        ...
        <div id="content-view" """
content = content[:m_html.start()] + html_payload + content[m_html.end()-20:]

# 3. MOCK DATA
m_demo = re.search(r'const demoDetails = \{\{.*?\}\};', content, re.DOTALL)
if not m_demo: print('Mock data not found'); sys.exit(1)
...
content = content[:m_demo.start()] + rich_demo_data.replace('{', '{{').replace('}', '}}') + content[m_demo.end():]

# 4. JS Panel features logic
if 'function showPanel' not in content:
...
with open(path, 'w', encoding='utf-8') as f:
    f.write(content)
print("Regex replace successfully completed!")
