...
# ── 2. HTML Injection ──
html_target = """    <div class="main-content">
...
if html_target not in content:
    print("HTML failed")
    sys.exit(1)
content = content.replace(html_target, html_replacement, 1)


# ── 3. JS Data Mock Replace ──
js_target_start = '                const demoDetails = {{'
js_target_end = '                renderMemberDetails(member, demoDetails);\n            }}\n        }}'

idx_js_start = content.find(js_target_start)
if idx_js_start == -1: print("JS mock start failed"); sys.exit(1)
idx_js_end = content.find(js_target_end, idx_js_start)
if idx_js_end == -1: print("JS mock end failed"); sys.exit(1)

rich_demo_data = """                const demoDetails = {
                    gowri: {
                        ...
                        "history": [ { "date": "2024-04-09", "score": 92, "event": "Clean Environment Scan" }, { "date": "2024-04-08", "score": 92, "event": "Updated Python to 3.12.2" }, { "date": "2024-04-06", "score": 88, "event": "Drift Resolved: Docker" } ]
                    },
                    aniket: {
                        ...
                        "history": [ { "date": "1 hour ago", "score": 85, "event": "Python version warning threshold" }, { "date": "Yesterday", "score": 88, "event": "Environment Synchronized" }, { "date": "2024-04-05", "score": 90, "event": "Passed Policy Checks" } ]
                    },
                    shuvam: {
                        ...
                        "history": [ { "date": "15 mins ago", "score": 70, "event": "Critical Drift: End-of-Life Node Version" }, { "date": "2024-04-07", "score": 75, "event": "Selenium upgrade failed" }, { "date": "2024-04-01", "score": 82, "event": "Routine sync" } ]
                    },
                    arun: {
                        ...
                        "history": [ { "date": "Just now", "score": 100, "event": "Perfect Synchronization" }, { "date": "2024-04-08", "score": 100, "event": "Provisioned new policies" }, { "date": "2024-04-02", "score": 100, "event": "Standardized Terraform" } ]
                    }
                }[member.id] || {
                    "bio": "Team member details not found.",
...
# ── 4. JS Panel Logic Injection ──
# We insert this right after `loadTeam();`
panel_logic = """
        // ── Feature Logic ────────────────────────────────────────────────
        ...
        loadTeam();
        setTimeout(() => showPanel('status'), 500);
"""

# Apply the mock data replacement strictly
content = content[:idx_js_start] + rich_demo_data.replace('{', '{{').replace('}', '}}') + content[idx_js_end + len(js_target_end):]

# Add panel logic right after loadTeam
end_idx = content.find("        loadTeam();")
content = content[:end_idx] + panel_logic + content[end_idx + len("        loadTeam();"):]
